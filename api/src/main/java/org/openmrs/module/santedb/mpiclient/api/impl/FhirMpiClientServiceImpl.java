...
public class FhirMpiClientServiceImpl implements MpiClientWorker, ApplicationContextAware {
	...
	public List<MpiPatient> searchPatient(String familyName, String givenName, Date dateOfBirth, boolean fuzzyDate,
		...
		if (familyName != null && !familyName.isEmpty())
			query = query.where(FAMILY.contains().value(familyName));
		if (givenName != null && !givenName.isEmpty())
			query = query.where(GIVEN.contains().value(givenName));
		if (dateOfBirth != null) {
			if (fuzzyDate) {
				if (this.m_configuration.getPdqDateFuzz() == 0) {
					query = query.where(
							BIRTHDATE.after().day(new Date(dateOfBirth.getYear(), 0, 1)));
					query = query.where(BIRTHDATE.before()
							.day(new Date(dateOfBirth.getYear(), 11, 31)));
				} else {
					query = query.where(BIRTHDATE.after()
				...
				query = query.where(BIRTHDATE.exactly().day(dateOfBirth));
		}

		if (gender != null && !gender.isEmpty())
			query = query.where(GENDER.exactly().code(gender));
		...
		if (patientIdentifier != null) {
			if (patientIdentifier.getIdentifierType() != null) {
				...
				query = query.where(IDENTIFIER.exactly()
		...
		if (stateOrRegion != null && !stateOrRegion.isEmpty())
			query = query.where(ADDRESS_STATE.contains().value(stateOrRegion));
		if (cityOrTownship != null && !cityOrTownship.isEmpty())
			query = query.where(ADDRESS_CITY.contains().value(cityOrTownship));
	...
	public MpiPatient getPatient(String identifier, String assigningAuthority) throws MpiClientException {
		...
		try {
			...
			Bundle results = this
					.getClient(true).search().forResource("Patient").where(IDENTIFIER
...
//            Set mother's name
			...
			Set<org.openmrs.Obs> patientObs = registrationEncounter.getObsAtTopLevel(false);
