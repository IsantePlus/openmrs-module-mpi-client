...
public final class MessageUtil {
	...
	private void updatePID(PID pid, Patient patient, boolean localIdOnly) throws HL7Exception, RegexSyntaxException {
		...
		for(Relationship rel : Context.getPersonService().getRelationships(patient))
		{
					...
					patient.equals(rel.getPersonB())) //MOTHER?
			{
				...
				pid.getMotherSIdentifier(0).getAssigningAuthority().getUniversalID().setValue(this.m_configuration.getLocalPatientIdRoot());
	...
	private void updateNK1(NK1 nk1, Relationship relationship, boolean localIdOnly) throws HL7Exception, RegexSyntaxException {
		...
		// Identity
		nk1.getNextOfKinAssociatedPartySIdentifiers(0).getAssigningAuthority().getUniversalID().setValue(this.m_configuration.getLocalPatientIdRoot());
		...
		if(person.getBirthdate() != null) {
			if(person.getBirthdateEstimated())
				...
				nk1.getDateTimeOfBirth().getTime().setValue(new SimpleDateFormat("yyyyMMdd").format(person.getBirthdate()));
		}
	...
	private void updateMSH(MSH msh, String messageCode, String triggerEvent) throws DataTypeException {
        ...
        msh.getReceivingApplication().getNamespaceID().setValue(this.m_configuration.getRemoteApplication()); // Client Registry
	 ...
	 * @throws HL7Exception 
	 */
	public List<MpiPatient> interpretPIDSegments(
			Message response
		...
		for(int i = 0; i < responseGroups.length; i++)
		{
			...
			for(Structure pidStruct : queryResponseGroup.getAll("PID")) // Parsing PID
			{
				...
				for(CX id : pid.getPatientIdentifierList())
				{
					...
					if(this.m_configuration.getLocalPatientIdRoot().equals(id.getAssigningAuthority().getNamespaceID().getValue()) ||
							this.m_configuration.getLocalPatientIdRoot().equals(id.getAssigningAuthority().getUniversalID().getValue()))
					{
			...
			if(Arrays.asList(queryResponseGroup.getNames()).contains("NK1") &&
				...
				for(Structure nk1Struct : queryResponseGroup.getAll("NK1"))
				{
					...
					RelationshipType relType = Context.getPersonService().getRelationshipTypeByName(nk1.getRelationship().getIdentifier().getValue());
					if(relType != null)
					...
					{
						relationship.setRelationshipType(Context.getPersonService().getRelationshipTypeByName("Parent/Child"));
					}
					else {
						this.log.warn(String.format("Can't convey relationship %s. Please create a relationship type with this name", nk1.getRelationship().getIdentifier().getValue()));
					}
